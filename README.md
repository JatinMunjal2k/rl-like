...
- Sound mixes samples and synthesis. Sampled (CC0 / CC-BY, see `public/CREDITS.md`): two engine loops crossfaded and pitched by speed, a boost roar with an ignition burst, three ball impacts and three car impacts picked at random with pitch variation and distance attenuation, soft and hard landings, big and small pad pickups, countdown tick and go, goal crunch and boom, a stadium crowd murmur that swells into a cheer on goals. Synthesised: jump, double-jump and dodge whooshes, tyre roll and skid, supersonic wind, the goal horn. Everything runs through a compressor so stacked impacts never clip, and every sample has a synthesised stand-in if it fails to load. Nothing is taken from Rocket League itself. Volume is in Settings → Gameplay.
